    ...
    <div className="p-8 max-w-[1400px] mx-auto animate-in fade-in duration-500 space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-edvoura-navy rounded-2xl p-8 text-white shadow-md">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white flex items-center gap-3">
            <LayoutGrid className="text-edvoura-gold w-8 h-8" /> Settings Dashboard
          </h1>
          <p className="mt-2 text-slate-300 text-sm">Secure enterprise-grade overview for Settings records and actions.</p>
        </div>
        <div className="mt-6 md:mt-0 flex gap-3">
          <Link href="/dash/admin/settings?export=report" className="inline-flex items-center justify-center rounded-md border border-slate-700 bg-slate-800 px-4 py-2 text-sm font-medium text-slate-200 transition-colors hover:bg-slate-700">
            Export Report
          </Link>
          <Link href="/dash/admin/settings?action=new-entry" className="inline-flex items-center justify-center rounded-md bg-edvoura-gold px-4 py-2 text-sm font-bold text-edvoura-navy-dark transition-colors hover:bg-yellow-400">
            New Entry
          </Link>
        </div>
      </div>
      ...
      <Card className="rounded-2xl shadow-sm border-slate-200 overflow-hidden">
        <CardHeader className="bg-slate-50 border-b border-slate-100 p-6 flex flex-row justify-between items-center">
          <CardTitle className="text-lg text-slate-800">Recent Settings Activity</CardTitle>
          <Link href="/dash/admin/settings?view=all" className="inline-flex h-8 items-center justify-center rounded-md bg-transparent px-4 py-2 text-xs font-bold text-edvoura-navy transition-colors hover:bg-slate-100">
            View All
          </Link>
        </CardHeader>
            ...
            <tbody className="divide-y divide-slate-50">
              {[1, 2, 3, 4].map((i) => (
                <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                  <td className="px-6 py-4 font-medium text-slate-800">REF-500{i}</td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <Link href={`/dash/admin/settings?action=review&id=REF-500${i}`} className="inline-flex h-7 items-center justify-center rounded-md border border-slate-300 bg-transparent px-4 py-2 text-[10px] font-medium text-slate-600 transition-colors hover:bg-slate-100">
                      Review
                    </Link>
                  </td>
                </tr>
