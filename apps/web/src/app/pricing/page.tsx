    ...
    <div className="flex flex-col min-h-screen bg-off-white selection:bg-yellow selection:text-navy font-body">
      ...
      >
        ...
        <div className="marketing-container relative z-10 pt-6 text-center">
          ...
          <p className="text-navy text-xl max-w-2xl mx-auto font-bold bg-white/70 backdrop-blur-md p-6 rounded-2xl border-4 border-navy shadow-[6px_6px_0px_#0A1628] transform -rotate-1">
        ...
        <div className="marketing-container">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 items-center lg:items-stretch">
            {plans.map((plan, i) => (
              <div
                key={plan.name}
                className={`rounded-[2rem] p-8 md:p-10 transition-transform duration-300 relative border-4 border-navy shadow-[12px_12px_0px_#0A1628] ${
                  plan.popular 
              ...
              >
                {plan.popular && (
                  <div className="absolute -top-6 left-1/2 -translate-x-1/2 bg-yellow border-4 border-navy text-navy text-sm font-black uppercase tracking-widest px-6 py-2 rounded-xl flex items-center gap-2 shadow-[4px_4px_0px_#0A1628] rotate-3">
                    <Star className="w-4 h-4 fill-navy" /> Most Popular
                  </div>
                )}
                
                <h3 className="font-heading font-black text-3xl mb-4">{plan.name}</h3>
                <p className={`text-lg font-bold mb-4 ${plan.popular ? 'text-white/80' : 'text-navy/80'}`}>{plan.desc}</p>
                <p className={`text-sm font-black uppercase tracking-widest mb-4 py-1 px-3 border-2 border-dashed rounded-lg inline-block ${plan.popular ? 'border-yellow/50 text-yellow' : 'border-navy/20 text-navy'}`}>
                  {plan.hourly} / hour
                </p>
                
                <div className="mb-10 bg-white/10 p-4 rounded-xl border-4 border-transparent">
                  <span className={`text-5xl font-heading font-black tracking-tight ${plan.popular ? 'text-yellow' : 'text-navy'}`}>{plan.price}</span>
                  <span className={`text-xl font-bold ml-1 ${plan.popular ? 'text-white/60' : 'text-navy/60'}`}>{plan.period}</span>
                </div>
                
                <Link
                  href="/signup"
                  className={`block text-center font-heading font-black py-4 rounded-xl transition-all text-xl mb-10 w-full ${plan.buttonCode}`}
                >
                  Choose {plan.name}
                </Link>
                  ...
                  <ul className="space-y-4">
                    {plan.features.map((f) => (
                      <li key={f} className="flex items-start gap-4 text-base font-bold">
                        <CheckCircle2 className={`w-6 h-6 shrink-0 mt-0.5 ${plan.popular ? 'text-yellow' : 'text-success'}`} />
                        <span className={plan.popular ? 'text-white' : 'text-navy'}>{f}</span>
                      </li>
                    ))}
                    {plan.excluded.map((f) => (
                      <li key={f} className={`flex items-start gap-4 text-base font-bold ${plan.popular ? 'opacity-40' : 'opacity-40'}`}>
                        <div className="w-6 h-6 shrink-0 mt-0.5 rounded bg-black/10 flex items-center justify-center">
          ...
          <div className="bg-error border-4 border-navy shadow-[15px_15px_0px_#0A1628] rounded-[3rem] p-12 text-white relative overflow-hidden transform rotate-1">
             ...
             <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
               <div className="max-w-2xl text-center md:text-left">
                  <h2 className="font-heading font-black text-4xl md:text-6xl mb-6 leading-tight drop-shadow-[4px_4px_0px_#0A1628]">Weekends & Assignment Support</h2>
                  <p className="text-xl font-bold opacity-90">Specialized on-demand support for complex assignments and intensive weekend sessions.</p>
               </div>
               <div className="bg-white text-navy border-4 border-navy rounded-[2rem] p-10 text-center shadow-[10px_10px_0px_#0A1628] transform -rotate-3 hover:rotate-0 transition-transform cursor-default">
                  <p className="text-sm font-black uppercase tracking-widest mb-2">Hourly Rate</p>
                  <p className="text-6xl font-heading font-black">$20<span className="text-xl"> /hr</span></p>
                  <Link href="/signup" className="mt-6 inline-flex items-center gap-2 bg-navy text-white font-heading font-black px-8 py-3 rounded-xl hover:bg-info transition-colors">
      ...
      <section className="bg-success py-24 md:py-32 border-b-8 border-navy relative">
        ...
        <div className="marketing-container relative z-10 max-w-4xl">
          <div className="text-center mb-16">
            ...
            <h2 className="font-heading font-black text-white text-5xl md:text-6xl drop-shadow-[4px_4px_0px_#0A1628]">
              Frequently Asked
            </h2>
          </div>
          ...
          <div className="space-y-6">
            {faqs.map((faq) => (
              <details key={faq.q} className="group bg-white border-4 border-navy shadow-[6px_6px_0px_#0A1628] rounded-2xl overflow-hidden transition-all hover:shadow-[8px_8px_0px_#0A1628] hover:-translate-y-1">
                <summary className="flex items-center justify-between p-6 md:p-8 cursor-pointer list-none">
                  <span className="font-heading font-black text-navy text-xl pr-4">{faq.q}</span>
                  <div className="w-12 h-12 bg-yellow rounded-xl flex items-center justify-center shrink-0 border-4 border-navy shadow-[2px_2px_0px_#0A1628] group-open:bg-navy group-open:border-navy group-open:text-white transition-colors">
                    <HelpCircle className="w-6 h-6 text-navy group-open:hidden" />
                    <ChevronRight className="w-6 h-6 hidden group-open:block group-open:rotate-90 transition-transform" />
                  </div>
                </summary>
                <div className="px-6 md:px-8 pb-8 text-navy text-lg font-bold leading-relaxed -mt-2 bg-off-white border-t-4 border-navy pt-6 shadow-[inset_0_4px_0_rgba(10,22,40,0.1)]">
                  {faq.a}
                </div>
              </details>
            ))}
