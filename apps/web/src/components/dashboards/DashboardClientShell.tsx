...
import { Activity, ArrowLeft, Bell, Crown, Search } from 'lucide-react';
  ...
  <Link href={href} className="relative group block">
    <motion.div
      className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-500 overflow-hidden ${
        active ? 'bg-yellow shadow-[0_0_20px_rgba(245,197,24,0.3)]' : 'hover:bg-white/5'
      }`}
      whileHover={{ x: 4 }}
    ...
    >
      ...
      <Icon
        className={`w-4 h-4 shrink-0 transition-transform duration-500 ${
          active ? 'text-navy scale-110' : 'text-grey group-hover:text-yellow'
        }`}
      />
      <span
        className={`text-xs font-black uppercase tracking-widest ${
          active ? 'text-navy' : 'text-grey group-hover:text-white'
        }`}
      >
        {label}
      </span>
      {active ? <div className="ml-auto w-1.5 h-1.5 bg-navy rounded-full animate-pulse" /> : null}
    </motion.div>
  </Link>
);
      ...
      <div className="min-h-screen bg-white flex font-body text-dark selection:bg-yellow selection:text-dark overflow-hidden uppercase font-black">
        <aside className="w-80 bg-navy border-r-[4px] border-dark sticky top-0 h-screen flex flex-col p-8 z-[60] pointer-events-auto transition-all duration-500 shadow-[10px_0px_0px_#F5C518]">
          <div className="flex items-center gap-4 mb-16 p-2 group cursor-pointer brutalist-3d bg-yellow border-[3px] border-dark shadow-[4px_4px_0px_#060E1C] rounded-xl">
            ...
            <div>
              <h2 className="font-heading font-black text-2xl tracking-tighter leading-none mb-1 text-navy">
                EDVOURA
              </h2>
              <span className="text-[10px] font-black tracking-[0.2em] text-navy/60 uppercase">
                High-Tier Learning
              </span>
            </div>
          </div>
          ...
          <nav className="space-y-6 flex-1 overflow-y-auto pr-2 custom-scrollbar">
            ...
            {role === 'student' ? <StudentSidebarNav initialBand={initialBand} /> : null}
          ...
          <div className="mt-auto pt-8 border-t-[3px] border-white/10 space-y-6">
            <div className="flex items-center gap-4 p-4 rounded-xl bg-white border-[3px] border-dark hover:shadow-[6px_6px_0px_#F5C518] transition-all group cursor-pointer transform hover:-translate-y-1">
              <div className="w-12 h-12 rounded-lg bg-yellow border-[2px] border-dark flex items-center justify-center p-1 overflow-hidden">
                <img
                  src={`https://api.dicebear.com/7.x/bottts/svg?seed=${encodeURIComponent(viewerName)}`}
                  alt="Avatar"
                  className="w-full h-full"
                />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-[11px] font-black uppercase tracking-tight text-dark truncate">
                  {viewerName}
                </p>
                <p className="text-[9px] font-black text-navy opacity-40 uppercase tracking-tighter">
                  {viewerSecondaryLabel}
                </p>
              </div>
            </div>
          ...
          <header className="h-24 bg-white border-b-[4px] border-dark px-12 flex items-center justify-between z-[50] pointer-events-auto">
            <div className="flex items-center gap-12">
              <div className="flex items-center gap-4 bg-white border-[3px] border-dark px-6 py-2 shadow-[4px_4px_0px_#22C55E] rounded-md">
                ...
                <span className="text-[11px] font-black text-dark uppercase tracking-widest">
                  Active Connection
                </span>
              </div>
            ...
            <div className="flex items-center gap-8">
              <div className="bg-white border-[3px] border-dark px-6 py-2 shadow-[4px_4px_0px_#060E1C] rounded-md flex flex-col items-end">
                ...
                <span className="text-[9px] font-black text-navy opacity-40 tracking-[0.2em] uppercase">
                  Universal Time
                </span>
              </div>
